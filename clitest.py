...
def normalize_output(text, normalize_rules):
    ...
    # Apply ANSI stripping first if present
    if "ansi" in rules:
    ...
    if "whitespace" in rules:
        # 1. Replace all whitespace characters (space, tab, newline, etc.) with a single space
...
def compare_streams(actual_output, expect_element):
    ...
    if expect_element is None:
        ...
        # FIX: Do not strip the expected text. The user is responsible for correctness,
        # including newlines, which are significant in command-line output.
...
def run_test_case(case_element, suite_env):
    ...
    # FIX: A test case should default to expecting exit code 0 unless specified otherwise.
...
def run_suite(suite_path, args, is_subtest=False):
    ...
    if suite_env_el is not None:
        ...
        # Prepare the full environment before running setup
...
def main():
    ...
    for i, suite_path in enumerate(args.suites):
        if is_multi_suite:
            suite_description = suite_path
            try:
                tree = ET.parse(suite_path)
                suite_description = tree.getroot().get("description", suite_path)
            except Exception:
                pass
            
            suite_result_code, num_tests = run_suite(suite_path, args, is_subtest=True)
            ...
            exit_code, num_tests = run_suite(suite_path, args, is_subtest=False)
